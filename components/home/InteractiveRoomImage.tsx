...
export function InteractiveRoomImage() {
  ...
  const [bubblePositions, setBubblePositions] = useState<{[key: string]: {x: number, y: number}}>({})
    ...
    >
      <div style={{
        ...
        <div style={{
          ...
          {/* Interactive Bubbles */}
                  {bubbles.map((bubble) => {
          const position = bubblePositions[bubble.id] || { x: bubble.x, y: bubble.y }
          return (
            <div
              key={bubble.id}
              style={{
                ...
                transition: isDragging === bubble.id ? 'none' : 'all 0.5s ease-out',
                zIndex: 10,
                pointerEvents: 'auto',
