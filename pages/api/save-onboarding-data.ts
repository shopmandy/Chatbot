...
export default async function handler(
  ...
  try {
    ...
    const { name, age, city, type_of_home, category_budgets, styles, brands } =
    ...
    if (!category_budgets || typeof category_budgets !== 'object') {
      return res
        .status(400)
        .json({ error: 'Missing or invalid category_budgets' })
    }
    ...
    if (error) {
      ...
      return res
        .status(500)
        .json({ error: error.message || 'Failed to save onboarding data' })
    }
