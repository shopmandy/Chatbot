...
export class RAGProcessor {
  ...
  private combineContext(query: string, documents: VectorSearchResult[]): string {
  ...
  async enhanceSystemPrompt(basePrompt: string, context: RAGContext): Promise<string> {
...
Current user question: ${context.query}`
  ...
  async searchKnowledgeBase(query: string, limit: number = 5): Promise<VectorSearchResult[]> {
    try {
      return await vectorStore.searchSimilar(query, limit, this.similarityThreshold)
    } catch (error) {
  ...
  getContextSummary(context: RAGContext): string {
    ...
    const sources = context.retrievedDocuments.map(doc => 
      doc.metadata.source + (doc.metadata.title ? ` - ${doc.metadata.title}` : '')
    )
